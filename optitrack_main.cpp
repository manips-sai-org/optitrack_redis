...
int main(int argc, char** argv) {
	...
	if (argc < 3) {
		std::cout << "Usage: optitrack_redis [-i INPUT_CSV_FILE]" << std::endl
		          ...
		          << "\n\nNo argument spacified, using local public ip -l 172.24.68.64 by default\n\n" << std::endl;
		// exit(0);
	}
	...
	while (true) {
		...
		for (int i = 0; i < optitrack.pos_rigid_bodies_.size(); i++) {
			pos_rigid_bodies.row(i) = optitrack.pos_rigid_bodies_[i];
		}
		for (int i = 0; i < optitrack.ori_rigid_bodies_.size(); i++) {
			auto& ori = optitrack.ori_rigid_bodies_[i];
